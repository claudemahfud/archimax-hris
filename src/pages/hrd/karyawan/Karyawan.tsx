...
export default function Karyawan() {
  ...
  // Deteksi NIP duplikat dari data yang SUDAH termuat (tidak perlu query tambahan) — supaya
  // HRD gampang lihat kalau ada sisa data lama duplikat, ketimbang baru ketahuan pas edit gagal.
  const grupNipDuplikat = useMemo(() => {
    const perNip = new Map<string, KaryawanType[]>();
    for (const k of daftar) {
      if (!k.nip) continue;
      const arr = perNip.get(k.nip) || [];
      arr.push(k);
      perNip.set(k.nip, arr);
    }
    return [...perNip.entries()].filter(([, arr]) => arr.length > 1);
  }, [daftar]);
    ...
    <div>
      ...
      <div className="page">
        ...
        <form className="card" onSubmit={handleSubmit}>
          ...
          <div className="form-grid">
            ...
            <div className="form-field span-3">
              ...
              <select id="levelUser" value={form.levelUser} onChange={(e) => updateField('levelUser', e.target.value as 'Staff' | 'HOD' | 'EKSEKUTIF' | 'Branch Manager')} required>
        ...
        <div className="card">
          ...
          {grupNipDuplikat.length > 0 && (
            <div style={{ background: '#fff3cd', border: '1px solid #ffe08a', borderRadius: 8, padding: '12px 16px', marginBottom: 16 }}>
              <strong>⚠ Ditemukan {grupNipDuplikat.length} NIP dipakai lebih dari satu karyawan:</strong>
              <ul style={{ margin: '8px 0 0', paddingLeft: 20 }}>
                {grupNipDuplikat.map(([nip, arr]) => (
                  <li key={nip} style={{ marginBottom: 4 }}>
                    NIP <strong>{nip}</strong> dipakai {arr.length}x oleh:{' '}
                    {arr.map((k, i) => (
                      <span key={k.id}>
                        {i > 0 && ', '}
                        {k.namaLengkap || '(nama kosong)'}{' '}
                        <button type="button" className="btn btn-secondary" style={{ padding: '2px 8px', fontSize: '0.8rem' }} onClick={() => mulaiEdit(k)}>Edit</button>
                      </span>
                    ))}
                  </li>
                ))}
              ...
              <p style={{ margin: '8px 0 0', fontSize: '0.85rem' }}>
                Cek satu per satu mana yang data valid, lalu ganti NIP salah satunya atau hapus yang duplikat/salah lewat tombol di tabel di bawah.
              </p>
            </div>
          )}
          {loading ? (
